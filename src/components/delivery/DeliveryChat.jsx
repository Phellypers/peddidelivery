...
export default function DeliveryChat({ orderId, customerName, customerEmail, delivererUserId, userType }) {
  const [messages, setMessages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [text, setText] = useState('');
  const [sending, setSending] = useState(false);
  const scrollRef = useRef(null);
  const conversationId = `delivery_${orderId}`;

  const load = async () => {
    const msgs = await base44.entities.ChatMessage.filter({ conversation_id: conversationId }, 'created_date', 200);
    setMessages(msgs);
    setLoading(false);
    const otherType = userType === 'customer' ? 'deliverer' : 'customer';
    const readField = userType === 'customer' ? 'is_read_by_customer' : 'is_read_by_deliverer';
    try {
      await base44.entities.ChatMessage.updateMany(
        { conversation_id: conversationId, sender_type: otherType, [readField]: false },
        { $set: { [readField]: true } }
      );
    } catch (_) {}
  ...
  useEffect(() => {
    ...
    const unsub = base44.entities.ChatMessage.subscribe((event) => {
      if (event.type === 'create' && event.data.conversation_id === conversationId) {
        ...
        const otherType = userType === 'customer' ? 'deliverer' : 'customer';
        if (event.data.sender_type === otherType) {
          const readField = userType === 'customer' ? 'is_read_by_customer' : 'is_read_by_deliverer';
          base44.entities.ChatMessage.update(event.data.id, { [readField]: true }).catch(() => {});
        }
  ...
  const send = async () => {
    ...
    await base44.entities.ChatMessage.create({
      ...
      is_read_by_customer: userType === 'customer',
      is_read_by_deliverer: userType === 'deliverer',
    });
    setText('');
    setSending(false);
  };
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-2 bg-gray-50">
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="text-xs">Envie uma mensagem para iniciar o chat</p>
          </div>
        ) : messages.map(m => (
          <div key={m.id} className={`flex ${m.sender_type === userType ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm ${m.sender_type === userType ? 'bg-blue-500 text-white' : 'bg-white border border-gray-100 text-gray-800'}`}>
              {m.message}
              <p className={`text-[9px] mt-0.5 ${m.sender_type === userType ? 'text-white/60' : 'text-gray-400'}`}>
