...
export default function Estoque() {
  ...
  const load = async () => {
    setError('');
    try {
      const data = await ingredientService.list('name');
      setIngredients(data);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (draft) {
      try {
        setEditing(JSON.parse(draft).ingredient || null);
        setShowForm(true);
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      setError(err.message || 'Não foi possível excluir o insumo.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && <IngredientForm ingredient={editing} onClose={() => { sessionStorage.removeItem('peddi_ingredient_draft'); setShowForm(false); }} onSave={() => { setShowForm(false); load(); }} />}
