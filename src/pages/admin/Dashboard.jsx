...
import { DollarSign, Eye, Package, Users, CheckCircle2, XCircle, TrendingUp, MapPin, BarChart3, LayoutList, Wallet, TrendingDown, Receipt } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (peddiApi.isConfigured) {
      Promise.all([loadAdminCatalog(), loadAdminReports(), base44.entities.CustomerProfile.list(), base44.entities.Account.list()])
        .then(([catalog, result, profiles, accounts]) => {
          setOrders(result.orders);
          setProducts(catalog.products);
          setCustomers(profiles);
          setAccounts(accounts);
        })
        .catch(error => setLoadError(error.message || 'Não foi possível carregar o painel.'))
  ...
  return (
    <div className="space-y-6">
      ...
      {peddiApi.isConfigured && <p className="text-sm text-muted-foreground">Dados do ambiente local. Os lançamentos financeiros registram os valores de teste; cobranças reais dependem da integração de pagamentos.</p>}
