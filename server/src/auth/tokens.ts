...
export type AuthUser = { id: string; email: string; name: string; role: string; businessId: string | null; storeId: string | null };
...
export function createAccessToken(user: AuthUser) {
  ...
  return jwt.sign({ sub: user.id, email: user.email, role: user.role, businessId: user.businessId, storeId: user.storeId }, env.jwtSecret,
